...
-   Vous comparez les résultats d'unités qui sont assignées à une condition de
    traitement et de celles qui sont assignées à une condition de contrôle.
:::
:::
...
## Each unit has both $Y_i(1)$ and $Y_i(0)$ \| *Chaque unité a les deux* $Y_i(1)$ et $Y_i(0)$
...
## We can take a random sample of these $Y_i(1)$ \| *Sélectionnons un échantionnage aléatoire de* $Y_i(1)$

\centering

![](../Images/fig06-y1-sample1.png){width="320"}

## We can take another random sample of these $Y_i(1)$ \| *Sélectionnons un autre échantionnage aléatoire de* $Y_i(1)$
...
## And we can take a random sample of these $Y_i(0)$ \| *Sélectionnons un autre échantionnage aléatoire de* $Y_i(0)$
...
-   Nous n'obtenons pas exactement $\overline{Y_i(1)}$ pour une assignation
    aléatoire particulière, mais nous pouvons attendre
...
-   L'assignation aléatoire crée deux groupes d'unités qui sont similaires
    excepté qu'une recevra le traitement et l'autre non. Les différences avant
    de recevoir le traitement ne sont dues qu'au hasard.

-  Les différences après le traitement peuvent étre attribuées au traitement
...
-   Important de maintenir la symétrie entre les groupes de
        traitement et de contrôle, excepté le traitement.
:::
:::
...
- Les différences peuvent être dues à des différences sous-jacentes (biais de sélection)
    ou au traitement (plus le hasard).  C'est souvent très difficile à dire.
