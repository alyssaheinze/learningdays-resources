...
::: {.col data-latex="{0.48\\textwidth}"}
-   Les resultats poténtiels

-   L'effet causale est une difference entre des resultats poténtiels

-   Les hypothèses clés pour l'inférence causale

-   Les différences entre les études aléotoires et les études observationelles
...
::: {.col data-latex="{0.48\\textwidth}"}
-   Vous avez une traitement.

-   *Vous assignez de manière aléatoire des sujets aux conditions de
    traitement et de contrôle.*

-   Vous comparez les résultats d'unités que sont assigné à condition de
    traitement de les résultats d'unités que sont assigné à condition de
    contrôle.
:::
:::
...
::: {.col data-latex="{0.48\\textwidth}"}
*L'effet causal moyen* de le traitement $T$ sur un résultat $Y$ pour
pour les unités que vous avez dans votre étude.

-   Vous devez définir la condition de contrôle pour définir un effet
    causal.
...
::: {.col data-latex="{0.48\\textwidth}"}
-   Nous utiliserons l'idée contrefactuelle.

-   "T cause Y" est une affirmation sur ce qui n'a pas eu lieu.

    -   "Sans T, il n'y a pas de Y."
...
-   "T cause Y" nécessite un *contexte*.
    ...
    -   Les petites salles de classe améliorent les résultats mais
...
# Learning the average causal effect from an experiment \| *Comment apprendre l'effet moyen d'une expérience aléatoires*
...
## $Y_i(1)$, the outcome each unit would have if treated \| $Y_i(1)$, le resultat qu'on obtiendrait si l'unité recevait le traitement

\centering

![](../Images/fig02-Y1.png){width="320"}

## $Y_i(0)$, the outcome each unit would have if not treated \| $Y_i(0)$, le resultat qu'on obtiendrait si l'unité recevait le contrôle

\centering

![](../Images/fig03-Y0.png){width="320"}

## Each unit has both $Y_i(1)$ and $Y_i(0)$ \| *Chaque unité a les deux* $Y_i(1)$ and $Y_i(0)$

\centering

![](../Images/fig02-Y1.png){width="220"}![](../Images/fig03-Y0.png){width="220"}

## So each unit has a treatment effect $\tau_i = Y_i(1) - Y_i(0)$ \| *Alors chaque unité a un effet de traitement* $\tau_i = Y_i(1) - Y_i(0)$
...
## Fundamental problem of causal inference \| *Problème fondamental de l'inférence causale*
...
-   Nous ne pouvons pas mesurer l'effet causal au niveau individuel, car
    nous ne pouvons pas observer à la fois $Y_i(1)$ et $Y_i(0)$ en même
    temps.
...
## We can take a random sample of these $Y_i(1)$ \| *Sélectons une échantionnage alèatoire de* $Y_i(1)$

\centering

![](../Images/fig06-y1-sample1.png){width="320"}

## We can take another random sample of these $Y_i(1)$ \| *Sélectons une autre échantionnage alèatoire de* $Y_i(1)$

\centering

![](../Images/fig07-y1-sample2.png){width="320"}

## And another! \| *Et une autre!*
...
## And we can take a random sample of these $Y_i(0)$ \| *Sélectons une autre échantionnage alèatoire de* $Y_i(0)$
...
## Randomization is powerful \| *Randomisation est puissante*

Average treatment effect / l'effet causal moyen (ATE):
...
::: {.col data-latex="{0.48\\textwidth}"}
2.  Parce que ces unités ont été assignées de manière aléatoire au
...
-   Nous ne obtenons pas $\overline{Y_i(1)}$ exact d'une assignation
    aléatoire particulière, mais nous pouvons nous attendre
...
::: {.col data-latex="{0.48\\textwidth}"}
-   L'assignation aléatoire crée deux gruppes d'unités qui sont similaires éxcepté qu'une recevra le traitement et l'autre non.  Les différences avant recevoir le traitement ne sont dues que le hasard.

-  Les différences aprés le traitement peuvent étre attribuées du traitement et plutôt que d'autres facteurs.  
:::
:::
...
1.  L'assignation aléatoire de traitement.
...
## Core assumption 1: Random assignment of treatment \| *Hypothèse clé 1: L'assignation aléatoire de traitement*
...
-   Çela ne signifie pas ni désordoné ni incontrôlé!
:::
:::
...
uniquement la reception du traitement par ce sujet. Il n'est pas affecté
par la façon dont les traitements sont assignés à d'autres sujets.
...
::: {.col data-latex="{0.48\\textwidth}"}
3.   L'assignation du traitement n'a aucun effet sur les résultats, sauf
    par l'effet de recevoir ou non le traitement.

-   Important de maintenir la symétrie entre les groupes de
        traitement et de contrôle, éxcepté le traitement.
:::
:::
...
- Nous pouvons attribuer les différences de résultats au traitement (plus
        le hasard).
:::
:::
...
- Les différences peuvent être dues à des différences sous-jacentes (biais de sélection) ou au traitement (plus le hasard).  Il est souvent très difficile à dire.
...
- Les études observationelles doivent invoquer d'autres hypothèses fortes pour faire la même affirmation d'un effet causal.
